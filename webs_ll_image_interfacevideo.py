...
if __name__ == '__main__':
    if len(sys.argv)!=10:
        print 'Usage: python scriptname cameraname path/to/folder hostname port password display[on|off] rectify[on|off] saveimages[on|off] savevideo[on|off]'
        exit(-1)
    ...
    display = sys.argv[6]
    rectify = sys.argv[7]
    saveimages = sys.argv[8]
    savevideo = sys.argv[9]
    ...
    while 1:
        try:
            ...
            if current_day!=filename[0:10]:
                ...
                date_N_days_ago_str = str(date_N_days_ago)
                date_N_days_ago_str = date_N_days_ago_str[0:10]
                fullpathtoremove = path + '/' + camera_name + '/' + date_N_days_ago_str 
                print 'removing'
                print fullpathtoremove
                try:
                    os.remove(fullpathtoremove+'/video.avi')
                    os.rmdir(fullpathtoremove)
                ...
                try:
                    ...
                    pass
                try:
                    videoname = fullpath+'/video.avi'
                    print videoname
                    if video is not None:
                        video.release()
                    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
                    video = cv2.VideoWriter(videoname,fourcc,3,(320,180))
                except OSError as exc:
                    print exc
                    raise exc
            ...
            if (display == 'on') | (rectify=='on') | (savevideo=='on'):
